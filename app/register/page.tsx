...
export default function RegisterPage() {
  ...
  const [otp_expiry_time, setOtpExpiryTime] = useState(0);
  ...
  useEffect(() => {
    if (!otp_expiry_time) return;

    const interval = setInterval(() => {
      const now = new Date().getTime();
      const expiryTime = new Date(otp_expiry_time).getTime();
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.data.success) {
        setStep(2);
        setOtpExpiryTime(res.data.data.otp_expiry_time);
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.data.success) {
        localStorage.setItem("userAuth", JSON.stringify(res.data.data.user));
        localStorage.setItem("api-token", JSON.stringify(res.data.data.token));
        // dispatch(login(res.data.data.user.name));
        setSuccess(true);
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        // backend should return otp_expiry_time
        setOtpExpiryTime(res.data.data.otp_expiry_time);
        setCanResend(false);
      } else {
              ...
              <>
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-6">
                  ...
                  <input
                    ...
                    className="w-full text-center tracking-widest text-xl py-3 border rounded-lg focus:ring-2 focus:outline-none"
